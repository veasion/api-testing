...
import cn.veasion.auto.exception.EvalException;
    ...
    private static Object parseGroup(List<SplitGroupUtils.Group> groupList, Object object) {
        ...
        for (SplitGroupUtils.Group g : groupList) {
            if (g.getChildren() != null && g.getChildren().size() > 0) {
                ...
                if (result == null) {
                    throw new EvalException(String.format("%s is null", g.getContext()));
                }
                sb.append(_L);
                if (isNumber(result)) {
                    sb.append(result);
    ...
    private static Object parseBrackets(final Object object, Object result, String text) {
        ...
        while (matcher.find()) {
            ...
            if (isNumber(text)) {
                ...
                if (varValue == null) {
                    throw new EvalException(String.format("%s 变量不存在 => %s", group, text));
                }
    ...
    private static Object parseArray(Object object, String text, int index) {
        if (object != null) {
            try {
                ...
                throw new EvalException(String.format("%s (%s) 不是一个 array 类型 => %s", object, object.getClass().getName(), text));
            } catch (ArrayIndexOutOfBoundsException e) {
                throw new EvalException(String.format("%s 数组越界 => %s", object, text));
            }
        }
        return null;
    }
